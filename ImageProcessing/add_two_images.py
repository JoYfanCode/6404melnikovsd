"""
Скрипт для сложения двух изображений из файлов
"""
import cv2
import numpy as np
import sys
...
def add_images(image1_path, image2_path, output_path):
    """
    Складывает два изображения и сохраняет результат
...
if __name__ == '__main__':
    if len(sys.argv) == 4:
        add_images(sys.argv[1], sys.argv[2], sys.argv[3])
    else:
        # Значения по умолчанию
        image1 = "lab2_images/1_Bengal_original.png"
        image2 = "lab2_images/1_Bengal_custom.png"
        output = "lab2_images/result_sum.png"
        
        print(f"Использование: python add_two_images.py <image1> <image2> <output>")
        print(f"\nСкладываем изображения по умолчанию:")
        print(f"  {image1}")
        print(f"  {image2}")
        print()
        
        add_images(image1, image2, output)
